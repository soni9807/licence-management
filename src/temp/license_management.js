...
import { cilCloudDownload, cilReload, cilList, cilSearch, cilChartPie } from '@coreui/icons'
// import FilteredSmartTable from 'src/tables/FilteredSmartTable'
import GenerateModal from './components/GenerateModal'
import GenerateModalAudit from './components//GenerateModalAudit'
import DisableModal from './components/DisableModal'
import RefreshModal from './components/RefreshModal'
import GenrateLincense from '../components/screens/GenrateLincense'
...
const Tenants = () => {
  ...
  const [isGenerateModalAuditOpen, setIsGenerateModalAuditOpen] = useState(false)
  ...
  const getLicenseInfo = async (search = false, key = false) => {
    setShowSpinner(true)
    let tenant_list_api_url = 'http://172.20.13.59:5004/get_snypr_license_audit'

    if (search === true) {
      tenant_list_api_url = config.VM_OPERATIONS_URL + `/list_tenants?search_key=${key}`
    }
  ...
  const onDisableModalOpen = (action, rowData) => {
    ...
    if (action.title === 'disable_license') {
      ...
      error_msg = 'Please select valid license to perfrom disable license operation.'
    } else if (action.title === 'enable_license') {
      allowed_status = 'invalid'
      error_msg = 'Please select invalid license to perfrom enable license operation.'
    }
  ...
  const onGenerateTenantModalOpen = (action, buttonName) => {
    ...
    if (buttonName === 'renew-license' && selectedRowData['Status'] === 'invalid') {
      let message = 'License is invalid. Please enable the license for editing/renewing.'
      let color = 'danger'
  ...
  return (
    <CContainer fluid style={{ backgroundColor: 'white', paddingTop: '1rem' }}>
      ...
      <CRow>
        <CCol sm={4}>
          {/* <CInputGroup style={{ marginLeft: 10, marginTop: 2 }}>
            <CInputGroupText color="info">
            ...
            <CFormInput
              ...
              onKeyPress={(event) => {
                if (event.key === 'Enter') {
                  getExcludedTenants(true, event.target.value)
                }
              }}
            />
          </CInputGroup> */}
        ...
        <CCol sm={7}>
          ...
          <CDropdown className="float-end">
            ...
            <CDropdownMenu>
              <CDropdownItem
                href={`ssh://${localStorage.getItem('username')}@${selectedTenants?.[0]?.['Spark Server IP']
                  }`}
              >
                Spark Server
              </CDropdownItem>
              <CDropdownItem
                href={`ssh://${localStorage.getItem('username')}@${selectedTenants?.[0]?.['App Server IP']
                  }`}
              >
                Application Server
              </CDropdownItem>
          ...
          <CDropdown className="float-end">
            ...
            <CDropdownMenu>
              {/* {jobsActions?.map((action) => (
                <CDropdownItem key={action.key} onClick={() => onModalOpen(action)}>
                  {action.label}
                </CDropdownItem>
              ))} */}
              {jobsActions?.map((action) => (
                  ...
                  {['generate-license'].includes(action.key) ? (
                    <>
                      ...
                      <CDropdownItem
                        ...
                        onClick={() => onGenerateTenantModalOpen(action, 'generate-license')}
                      >
                        {action.label} {action?.new ? <CBadge color="success">new</CBadge> : <></>}
                      </CDropdownItem>
                    </>
                    ...
                    <>
                      ...
                      <CDropdownItem
                        disabled={selectedTenants?.length !== 1}
                        key={action.key}
                        onClick={() => onGenerateTenantModalOpen(action, 'renew-license')}
                      >
                        {action.label}
                      </CDropdownItem>
                    </>
                    ...
                    <>
                      ...
                      <CDropdownItem
                        disabled={selectedTenants?.length === 0}
                        key={action.key}
                        onClick={() => onDisableModalOpen(action, 'disable-license')}
                      >
                        {action.label}
                      </CDropdownItem>
                    </>
                    ...
                    <>
                      ...
                      <CDropdownItem
                        disabled={selectedTenants?.length === 0}
                        key={action.key}
                        onClick={() => onDisableModalOpen(action, 'enable-license')}
                      >
                        {action.label}
                      </CDropdownItem>
                    </>
                    ...
                    <>
                      ...
                      <CDropdownItem
                        disabled={selectedTenants?.length !== 1}
                        key={action.key}
                        onClick={() => onRefreshModalOpen(action, 'refresh-license')}
                      >
                        {action.label}
                      </CDropdownItem>
                    </>
